...
function	drawStatus(dataRows)
{
	...
	*/
	const	status: StatusType[] = [
		{label: "Total de estudantes", value: dataRows.length},
		{label: "Trabalhando", value: dataRows.filter((row) => row.isWorkin == "Sim").length},
		{label: "Não Trabalhando", value: dataRows.filter((row) => row.isWorkin == "Não").length},
		{label: "Salário Médio", value: getAverange(dataRows)},
	];
	...
	return (
		<div className="flex flex-col w-full gap-4">
			<div className="flex bg-white w-full gap-4 p-4 border border-b rounded-md">
				<Input className="w-64" value={filter.name} placeholder="Buscar estudante..."
