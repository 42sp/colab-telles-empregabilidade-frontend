    ...
    async function fetchActiveOperations() {
      try {
        ...
        if (mounted) {
          ...
          futureOps.forEach((op: Operation) => {
            const isRecurring = Boolean(op.repeat_time && op.repeat_days);
            recurrenceState[op.uuid] = isRecurring;

            // Log detalhado da operação
            console.log(`Operação carregada:`);
            console.log(`- UUID: ${op.uuid}`);
            console.log(`- Nome: ${op.name}`);
            console.log(`- Data agendada: ${op.scheduled_date}`);
            console.log(`- Hora repetição: ${op.repeat_time}`);
            console.log(`- Dias repetição: ${op.repeat_days}`);
            console.log(`- Recorrente: ${isRecurring}`);
            console.log(`- Outros dados:`, op);
          });
