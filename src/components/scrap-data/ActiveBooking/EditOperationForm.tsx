...
export function EditOperationForm({ isSubmitting, operationId }: EditOperationFormProps) {
  ...
  async function handleSubmit(data: any) {
    ...
    try {
      await scrapService.patch(operationId, {
        ...data,
  ...
  return (
    <Card role="form" aria-labelledby="edit-operation-title" onSubmit={form.handleSubmit(handleSubmit)}>
      <CardHeader>
      ...
      <CardContent>
        <div className="flex flex-col gap-8">
          ...
          <div className="p-4 border rounded-md bg-gray-50 text-sm text-gray-700 flex flex-col gap-1">
            ...
            {last_edited_by && (
              <span>
                Última edição: <b>{last_edited_by}</b> em {last_edited_at || "-"}
              </span>
            )}
          </div>

          <div className="flex justify-end">
            <Button type="submit" disabled={isSubmitting} className="h-[42px] min-w-[130px]">
